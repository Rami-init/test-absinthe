defmodule BookingWeb.Resolvers.Vacation do
  ...
  def cancel_booking(_, %{booking_id: booking_id}, %{context: %{current_user: user}}) do
    ...
    if booking.user_id != user.id do
      ...
      case Booking.Vacation.cancel_booking(booking) do
        {:ok, canceled_booking} ->
          publish_booking_cancelled(canceled_booking)
          {:ok, canceled_booking}
  ...
  defp publish_new_booking(booking) do
    Absinthe.Subscription.publish(
      BookingWeb.Endpoint,
      booking,
      new_booking: "place:#{booking.place_id}"
    )
  end

  defp publish_booking_cancelled(booking) do
    Absinthe.Subscription.publish(
      BookingWeb.Endpoint,
      booking,
      booking_cancelled: "place:#{booking.place_id}"
    )
  end
end
